...
class Tool extends BasicApi
{
     ...
     * @var string
     */
    public $table_region = 'ApiDataRegion';   //基础地区数据表
    ...
    public function getVcodeByEmail()
    {
        ...
        $body = "
        尊敬的用户:<br>
        您好!您本次的验证码为:['vcode' => <font color='red'>{$v_code}</font>]<br>
        打死也不要告诉别人哦!<br>";
        $body.='<br>
        '.sysconf('site_name').'<br>
        '.date('Y-m-d H:i:s');

        $res = MailService::sendMail($email,'userid','邮件验证码',$body);
        $this->debug([
            'Time' => date('Y-m-d H:i:s'),
            'EmailCode' => $v_code,
        ]);
        if (!$res) {
            ...
            return $this->buildFailed(ReturnCode::VCODE_GET_FAILED, '发送失败');
        } else {
            // 验证码发送成功设置cache
            Cache::set($email . '_vcode', $v_code, config('api.V_CODE_TIME'));
            return $this->buildSuccess([
                'EmailMsg' => '验证码获取成功,请注意查收,如未收到，请查看垃圾箱或重新获取',
            ]);
        }
    }
