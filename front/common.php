...
function userLogin($account,$password){
    $api_url = config('api.API_URL').'/api/5c667f2f32c13';
    $data    = ['account'=>$account,'password'=>$password];
    $option  = [
        'header' => ['version:'.sysconf('app_version')]
    ];
    $res = HttpService::post($api_url,$data,$option);
    $res = json_decode($res,true);
    if($res['code']==1) {
        session('account',$res['data']['userInfo']);
        return $res;
    } else{
        return false;
    }
