...
class Row extends BasicAdmin
{
    ...
    {
        $apiRoutePath = env('root_path') . 'route/apiRoute.php';
        $tplPath = env('root_path') . 'extend/util/apiRoute.tpl';
        ...
        $foot = PHP_EOL . '        // 接口Hash异常跳转' . PHP_EOL . '        ]);' . PHP_EOL . ' Route::miss(\'api/miss/index\'); ' . PHP_EOL . '});';
        foreach ($listInfo as $value) {
            $tplStr .= '        // ' . $value->info . ' ' . $value->hash . PHP_EOL . '        \'' . $value->hash . '\' => [' . PHP_EOL . '            \'api/' . $value->apiName . '\',' . PHP_EOL . '            [\'method\' => \'' . $methodArr[$value->method] . '\', \'after\' => $afterBehavior]' . PHP_EOL . '        ],' . PHP_EOL;
        }
