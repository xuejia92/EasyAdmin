...
class Channel extends BasicAdmin
{
    ...
    protected function _index_data_filter(&$data)
    {
        foreach ($data as &$vo) {
            ...
            $vo['tname'] = Db::name('CmsSiteType')->where('id',$vo['tid'])->value('title');
        }
    ...
    protected function _form_filter(&$vo)
    {
        if ($this->request->isGet()) {
            ...
            $model_list = Db::name('CmsSiteType')->where('status','=',1)->select();

            $this->assign(['nodes' => array_column($nodes, 'node'), 'menus' => $menus,'model_list'=>$model_list]);
        }
    }
