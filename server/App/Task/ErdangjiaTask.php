 ...
 * @package App\Task
 */
class ErdangjiaTask extends AbstractAsyncTask
{
    /**
     * 执行投递
     * @param $taskData
    ...
    public function leeSign($username,$password){
        ...
        // 请求登录系统
        $request = new Request('https://www.erdangjiade.com/Ajax/checkLogin');
        $new_headers = $headers;
        $new_headers->setHeader('Referer', 'https://www.erdangjiade.com/login.html')
        ...
        $new_headers = $headers;
        $new_headers->setHeader('Referer', 'https://www.erdangjiade.com/login.html');
//        $token_request = new Request('https://www.erdangjiade.com/Member/sign');
...
//        $enter->token = $token[1];
        // 请求操作签到接口
        $signDayRequest = new Request('https://www.erdangjiade.com/Ajax/signDay');
        $new_headers = $headers;
        $new_headers->setHeader('Referer', 'https://www.erdangjiade.com/Member/sign');
        $signDayResponse = $signDayRequest->setCookies($cookies)
            ->setHeaders($new_headers)->exec();
        $result = $signDayResponse->getBody();
        if ($result === 'key') return  ('签到令牌错误: ');
